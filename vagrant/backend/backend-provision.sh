...
exec > >(tee -a /var/log/provision-backend.log) 2>&1
set -euo pipefail

##### 0) Vagrant env에서 내려온 값들(없으면 기본값) #####
NODE_IP="${NODE_IP:-192.168.4.103}"       # 이 노드 IP (kubelet --node-ip)
...
# 라벨/테인트(선택)
NODE_LABELS="${NODE_LABELS:-role=backend,tier=app}"
NODE_TAINTS="${NODE_TAINTS:-}"            # 예: "dedicated=backend:NoSchedule"
